<template>
    <div class="modal fade mt-5" id="addTransactioModal" tabindex="-1" aria-hidden="true"
        data-target=".bd-example-modal-lg" data-keyboard="false" data-backdrop="static">
        <div class="modal-dialog mt-0 ">
            <form @submit.prevent="save">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ modalType }} Transaction</h5>
                            ...
                            <div class="row no-gutters">
                                <div class="col-12">
                                    <!-- <div class="form-group">
                                        <label class="mb-0">Account ID <sup>*</sup></label>
                                        <input type="text" v-model="data.title" class="form-control" placeholder="Account Title" required readonly>
                                    </div> -->

                                    <div class="form-group">
                                        <label class="mb-0">Account<sup>*</sup></label>
                                        <select class="form-control" v-model="data.account_id">
                                            <option value="" selected disabled>Select Account</option>
                                            <option v-for="obj in accounts" :key="obj.id" :value="obj.id" required>
                                                {{ obj.title }} - {{ obj.name }}
                                            </option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label class="mb-0">Amount<sup>*</sup></label>
                                        <input type="text" v-model="data.amount" class="form-control"
                                            placeholder="Amount" required>
                                    </div>
                                    <div class="form-group">
                                        <label class="mb-0">Narration</label>
                                        <textarea type="text" v-model="data.narration" rows="5" class="form-control"
...
export default {
    ...
    methods: {
        save() {
            this.loading = true;
            let url = this.modalType == 'Add' ? '/admin/transactions' : `/admin/transactions/${this.objId}`;
        ...
        getDataForEdit() {
            this.loading = true
            axios({
                url: `/admin/transactions/${this.objId}/edit`,
                method: 'GET',
            })
                .then(response => {
                    this.loading = false
        ...
        getFormData() {
            axios({
                url: `/admin/transactions/get/form/data`,
                method: 'GET',
            })
                .then(response => {
