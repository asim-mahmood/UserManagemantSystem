<template>
    <div class="modal fade mt-5" id="addHeadModal" tabindex="-1" aria-hidden="true" data-target=".bd-example-modal-lg"
        data-keyboard="false" data-backdrop="static">
        <div class="modal-dialog mt-0 ">
            <form @submit.prevent="save">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ modalType }} Head</h5>
                            ...
                            <div class="row no-gutters">
                                <div class="col-12">
                                    <div class="form-group">
                                        <label class="mb-0">Dead<sup>*</sup></label>
                                        <input type="text" v-model="data.name" class="form-control"
                                            placeholder="Head Name" required>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        save() {
            this.loading = true;
            let url = this.modalType == 'Add' ? '/admin/heads' : `/admin/heads/${this.objId}`;
        ...
        getDataForEdit() {
            ...
            this.loading = true
            axios({
                url: `/admin/heads/${this.objId}/edit`,
                method: 'GET',
            })
                .then(response => {
                    this.loading = false
